...
% obs, model, misfits
% For M1 or M2 (edit Line 19, 24, 29)
...
% Load model outputs
load ../output/export_M1_low.mat % low
...
load ../output/export_M1_mid.mat % mid
...
load ../output/export_M1_high.mat % high
...
opt.clevs = -100:10:200 ;
opt.dash_clevs = -100:50:200 ;
opt.yTicks = []; 
opt.yTickLabels = []; 
...
opt2.clevs = 2050:10:2500 ;
opt2.dash_clevs = 2100:50:2500 ;
opt2.yTicks = []; 
opt2.yTickLabels = [];
...
opt3.clevs = -275:10:225 ;
opt3.dash_clevs = [];%-75:25:75 ;
opt3.yTicks = []; %-6000:1000:0 ;
opt3.yTickLabels = [];%cellstr(num2str((6:-1:0)')) ;
opt3.xTicks = [-89 -45 0 45 89];
opt3.xTickLabels = {'SP';'45°S';'EQ';'45°N';'NP'} ;
opt3.ylabel = []; %'Depth (km)' ;
opt3.xlabel = []; %'Latitude (°)' ;
...
set(gca,'FontSize',18,'CLim',[-150 150],'xdir','reverse');
ylabel('Depth (km)'); xlim([-89 89]);
text(82,-5500,'ATL','FontSize',18);
yticks([-6000 -4000 -2000 0]);
yticklabels({'6','4','2','0'}) ;
...
cmocean('balance')
text(50,-5500,'PAC','FontSize',18); xlim([-89 89]);
set(gca,'FontSize',18,'CLim',[-150 150])
set(gca,'box','off')
colorbar
