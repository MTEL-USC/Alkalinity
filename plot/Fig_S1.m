...
set(gca,'clim',[0 6],'fontsize',14);
title('P uptake')
setm(gca,'MapProjection','miller','MLabelParallel','south','fontsize',14)
...
subplot(4,2,5)
worldmap([-80 80],[1 359])
datariv = RIVER(:,:,1).*SURFACE; datariv(datariv==0)=NaN;
ind1 = find(~isnan(datariv)); LAT=ao.LAT(:,:,1); LON=ao.LON(:,:,1);  
scatterm(LAT(ind1),LON(ind1),80,datariv(ind1),'filled','Linewidth',2); hold on;
set(gca,'clim',[0 400],'fontsize',14);
...
subplot(4,2,6)
worldmap([-80 80],[1 359])
contourfm(ao.lat,ao.lon,buro_map.*SURFACE,60,'LineStyle','none'); hold on;
set(gca,'clim',[0 2],'fontsize',14);
...
subplot(4,2,7)
worldmap([-80 80],[1 359])
contourfm(ao.lat,ao.lon,burc_map.*SURFACE,600,'LineStyle','none'); hold on;
set(gca,'clim',[0 2],'fontsize',14);
...
subplot(4,2,8)
worldmap([-80 80],[1 359])
contourfm(ao.lat,ao.lon,bur_map,600,'LineStyle','none'); hold on;
set(gca,'clim',[0 2],'fontsize',14);
title('Total burial')
setm(gca,'MapProjection','miller','MLabelParallel','south','fontsize',14)
geoshow('landareas.shp','FaceColor', [0.7 0.7 0.7]); hold on;
