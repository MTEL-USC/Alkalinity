function [A,b,output] = arag(A,b,do)
% Aragnite dissolution: kinetics + respiration (M3)
fprintf('%s','arag...')
...
EPROD = PPROD.*CtoP.*ARAGPOC.*SF.*2; % 106 is the  C:P ratio; 2 is to double ALK from PIC
eprod = EPROD(ao.iocn);
...
ind1=find(OAeff<1 & OAeff>=0.85); 
Kdiss(ind1) = 0.0026.*(1-OAeff(ind1)).^0.13;

ind2=find(OAeff<0.85 & OAeff>=0);
Kdiss(ind2) = 0.028.*(1-OAeff(ind2)).^1.5;
